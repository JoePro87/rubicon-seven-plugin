...
### Canon Hardening (2026-07-24, commits `20da47e` + `2081afc`)

Built table-side after the Day-136 poisoning class (fabricated geography + NPC-attributed claims across ~10 out-of-character turns). Spec: `docs/handoffs/CANON_GATE_HARDENING_SPEC_2026-07-24.md`; ship handoff: `docs/handoffs/CANON_HARDENING_SHIP_HANDOFF_2026-07-24.md`. Verified against shipped code:

- **Ledger truncation killed (`20da47e`).** `MapSystem._ledger_append` used to store `fact[:300]` — silently slicing every revealed fact mid-word *while the success echo confirmed the full input text* (31 of 56 Thyricost entries damaged; the anti-fabrication store was lying about its own contents). Now: facts store INTACT; `LEDGER_FACT_MAX_CHARS = 2000` and an over-cap fact is REFUSED loudly (nothing written, "split it into two reveals"); the ✅ echo reads back out of `state["revealed_ledger"]` so any storage discrepancy surfaces in the confirmation instead of being masked by it. Recovery of the 31 damaged live entries is OPEN and needs DM review per entry (only source: chat transcripts; backups/git carry the same damage — see `docs/handoffs/LEDGER_TRUNCATION_FIXED_2026-07-24.md`).
- **Spatial-claim gate (`2081afc`, blocking).** Bearings/distances/containment about canon places must trace to a `geography()`/`map()`/`check_canon()` call this turn. `hooks/place_lexicon.py` builds the canon-place lexicon; `hooks/spatial_claim_gate.py` scans; `hooks/spatial_register.py` adds a session-scoped ADVISORY consistency register (sourced bearings that contradict earlier sourced bearings print to stdout, never block). Campaign-side tuning valve: `spatial_gate_config.json` (`exempt_place_names` etc. — fail-open on missing/malformed file, same pattern as `fabrication_tripwires.json`). Measured arm rate over a 200-turn live corpus: 6.0%.
- **Attributed-claim gate (`2081afc`, blocking).** A claim put in a canon NPC's mouth must be on the record (distillation cache + Revealed Ledger answer-key blob, mtime-cached) or sourced by a tool call this turn. Roster from `npc_states.json` + party names; fail-open with no roster. Measured arm rate: 7.0% (under the spec's 10% blocking threshold).
- **Out-of-character coverage.** Both gates share `_gate_preconditions`: skip under `in_maintenance`, on meta-only responses, and outside `session_type == "gameplay"`. The old C.2 meta-signal skip was DELETED and the C.3 conditional narrowed — deliberately, because the poisoning happened in out-of-character exposition where every earlier gate stood down. `test_hook_redesign`'s `SANCTIONED_BLOCKING_CHECKS` now carries `_check_spatial_source` + `_check_attributed_claims`. Cost: ~100 ms/turn warm (dominated by `/mnt/c` filesystem stats). Corpus-regression fixture `tests/fixtures/prose_window_200.jsonl` is dev-repo-only (ship-excluded); its tests skip when absent.

Tests: `tests/test_spatial_claim_gate.py`, `tests/test_attributed_claim_gate.py`, `tests/test_place_lexicon.py`, `tests/test_spatial_register.py`, `tests/test_ooc_coverage.py`, extended `tests/test_reveal_ledger.py`.
