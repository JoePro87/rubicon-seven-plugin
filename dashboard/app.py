...
class DashboardApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with TabbedContent(initial="party"):
            with TabPane("Party", id="party"):
                yield VerticalScroll(Static(model.NO_VIEW_PLACEHOLDER, id="party-body"))
            with TabPane("Map", id="map"):
                yield VerticalScroll(Static(model.NO_VIEW_PLACEHOLDER, id="map-body"))
            with TabPane("World", id="world"):
                yield Static(model.NO_VIEW_PLACEHOLDER, id="world-body")
            with TabPane("Parleys", id="parleys"):
                yield Static(model.NO_VIEW_PLACEHOLDER, id="parleys-body")
        yield Footer()
    ...
    def _poll(self) -> None:
        ...
        view_mtime = view_path.stat().st_mtime if view_path.exists() else None
        map_mtime = map_path.stat().st_mtime if map_path.exists() else None
    ...
    def _render_party(self) -> None:
        ...
        lines = []
        for c in cards:
            lines.append(f"{c['name']}   HP {c['hp_text']}   AV {c['av']}   "
                         f"Wounds {c['wounds']}   Slots {c['slots_text']}")
            if c["items"]:
                by_where = {}
                for it in c["items"]:
                    by_where.setdefault(it.get("where") or "carried", []).append(it)
                lines.append("  Items:")
                for where, its in by_where.items():
            ...
            lines.append("")
        body.update("\n".join(lines).rstrip() + self._suffix())
    ...
    def _render_map(self) -> None:
        ...
        if self._last_map_text is None:
            ...
            body.update(self._last_map_text + self._suffix())
